  ...
  return (
    <div className="bg-white rounded-xl border border-[#E7E5E4] p-4">
      <div className="flex justify-between items-start mb-2">
        <div>
          <p className="font-semibold text-[#1C1917]">Table {order.table_id}</p>
          <p className="text-xs text-[#78716C]">Commande #{order.id}</p>
        </div>
      ...
      <ul className="text-sm text-[#1C1917] mb-3 space-y-1">
        {order.items.map((item) => (
          <li key={item.id}>
            {item.quantity}× article #{item.menu_item_id}
            {item.note && <span className="text-[#78716C]"> — {item.note}</span>}
          </li>
        ))}
      </ul>

      <div className="flex justify-between items-center">
        <span className="font-medium text-[#1C1917]">{order.total.toFixed(2)} DT</span>
        {next && (
          <button
