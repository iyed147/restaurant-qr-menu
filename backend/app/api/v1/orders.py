...
from app.schemas.order import OrderCreateIn, OrderOut, OrderItemOut
...
def create_order(payload: OrderCreateIn, db: Session = Depends(get_db)):
    ...
    if existing:
        ...
        return OrderOut(
            ...
            items=[OrderItemOut.model_validate(i) for i in existing_items],
        )
    ...
    try:
        ...
        if again:
            ...
            return OrderOut(
                id=again.id,
                restaurant_id=again.restaurant_id,
                table_id=again.table_id,
                client_session_id=again.client_session_id,
                status=again.status.value if hasattr(again.status, "value") else str(again.status),
                total=float(again.total),
                idempotency_key=again.idempotency_key,
                items=[OrderItemOut.model_validate(i) for i in again_items],
            )
